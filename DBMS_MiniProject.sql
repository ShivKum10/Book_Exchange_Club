...
FROM
    Transaction t
JOIN
    BorrowRequest br ON t.request_id = br.request_id
JOIN
    Member m ON br.member_id_requester = m.member_id
JOIN
...
CALL ApproveBorrowRequest('BR011', 'A001');
CALL ReturnBook('T008');
CALL GetPrioritizedRequestList('B005');
CALL DenyBorrowRequest('BR013');
SELECT IsBookAvailable('B001') AS 'Is Book B001 Available?';
SELECT GetMemberActiveLoanCount('M005') AS 'Ethan Hunt Active Loans';
-- When you approve a request, a transaction is inserted, which fires the trigger.
CALL ApproveBorrowRequest('BR010', 'A002');
-- The ReturnBook procedure performs an UPDATE, firing the trigger.
-- Because the return is on time, no strike is issued.
CALL ReturnBook('T009');
-- A direct UPDATE also fires the trigger.
-- Because the return_date is after the due_date, a strike WILL be issued.
UPDATE Transaction
SET return_date = '2025-12-01' -- Assuming due_date was in November
WHERE transaction_id = 'T007';


CREATE USER 'library_admin'@'localhost' IDENTIFIED BY 'library123';
